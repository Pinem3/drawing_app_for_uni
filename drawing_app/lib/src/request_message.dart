import 'user.dart';
enum RequestType {
  all, add, update, delete, get;

  static RequestType fromString(String value) {
    try{
      return RequestType.values.firstWhere((element) => element.toString().split('.').last == value,);
    } catch (_) {
...
class UpdateUserRequest extends RequestMessage {
  ...
  factory UpdateUserRequest.fromJson(Map<String, dynamic> json,) {
    return UpdateUserRequest(User.fromJson(json['user']),);
  }
...
class DeleteUserRequest extends RequestMessage {
  ...
  DeleteUserRequest(this.id, {super.type = RequestType.delete});
...
class GetUserRequest extends RequestMessage {
  final int id;
  GetUserRequest(this.id, {super.type = RequestType.get});
